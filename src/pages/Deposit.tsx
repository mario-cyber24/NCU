...
import { Phone, CreditCard, Landmark, Clock, ArrowDown, Info } from 'lucide-react';
...
export default function Deposit() {
  ...
  const getInstructions = () => {
    switch (activeTab) {
      case 'wave': return waveInstructions;
      case 'aps': return apsInstructions;
      case 'bank': return bankInstructions;
      default: return waveInstructions;
    }
  };
    ...
    <div className="max-w-7xl mx-auto">
      <div className="md:flex md:items-center md:justify-between mb-6">
        ...
        <div className="mt-3 md:mt-0 bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
          ...
          <span className="ml-2 font-semibold text-gray-900">D{balance.toFixed(2)}</span>
        </div>
      </div>
        ...
        <div className="flex items-start">
          ...
          <div className="ml-4">
            ...
            <p className="mt-1 text-sm text-gray-600">
              When making deposits, always ensure you include your account details as reference to ensure your deposit is properly credited.
            </p>
            <div className="mt-3 bg-gray-50 p-3 rounded-md border border-gray-200">
              <p className="text-sm font-medium text-gray-700">Your Account Reference:</p>
              <p className="text-lg font-bold text-primary-700 mt-1">{user?.id.substring(0, 8).toUpperCase()}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            {depositMethods.map((method) => (
              ...
              >
                <div className="flex items-center justify-center">
                  <div className={`w-8 h-8 rounded-full ${method.color} flex items-center justify-center text-white mr-2`}>
                    {method.icon}
                  </div>
            ...
            <div className="w-full md:w-1/3 mb-6 md:mb-0">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-center mb-4">
                  <div className={`w-10 h-10 rounded-full ${depositMethods.find(m => m.id === activeTab)?.color} flex items-center justify-center text-white mr-3`}>
                    <ArrowDown size={20} />
                  </div>
                  <div>
                    <h4 className="font-medium text-gray-900">Deposit Method</h4>
                    <p className="text-sm text-gray-600">{depositMethods.find(m => m.id === activeTab)?.name}</p>
                  </div>
                </div>
                
                <div className="space-y-3">
                  <div className="flex">
                    <div className="w-24 flex-shrink-0 text-sm text-gray-500">Fee:</div>
                    <div className="text-sm font-medium text-gray-900">{depositMethods.find(m => m.id === activeTab)?.fee}</div>
                  </div>
                  <div className="flex">
                    <div className="w-24 flex-shrink-0 text-sm text-gray-500">Processing:</div>
                    <div className="text-sm font-medium text-gray-900 flex items-center">
                      <Clock size={14} className="mr-1" />
                      {depositMethods.find(m => m.id === activeTab)?.processing}
                    </div>
                  </div>
                  <div className="flex">
                    <div className="w-24 flex-shrink-0 text-sm text-gray-500">Available:</div>
                    <div className="text-sm font-medium text-gray-900">{depositMethods.find(m => m.id === activeTab)?.availability}</div>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-2 text-sm text-yellow-700">
                      <p>
                        Always keep your transaction receipt until the funds are reflected in your account.
                        If you encounter any issues, please contact our support team at support@naweccu.com.
                      </p>
                    </div>
                  </div>
                </div>
